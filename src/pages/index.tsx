...
function Home({ chatrooms }: HomeProps) {
  ...
  const chatroomList = chatrooms.map(room => {
    ...
    return (
      <li key={ _id } className='m-2 text-neutral list-none'>
        ...
        <button onClick={ () => joinRoom(_id) } className='btn btn-secondary btn-sm rounded-full m-2 text-base-100'>Click to Join</button>
      </li>
    )
  })
